...
- **`get` sub-path leaf typo 对封闭复合类型 fail-loud**（#157）：`get <node> position:typo` 读 Vector2/3/4 系（含 i 变体）的不存在 leaf 时现报 `1002 PROPERTY_NOT_FOUND`（message 列出合法 leaf），不再静默返回 `{"value": null}`（与真 null 不可区分的 footgun）。仅对 leaf 集能 100% 枚举完整的 vector 系生效；Color / Transform 等开放/未收录类型保持现状 `get_indexed` 行为（零回归，误杀比静默更坏）。
