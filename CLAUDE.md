...
*（截至 2026-06-06，已 land：#96/#97/#99/#100 三连（wait 原语/codec/多属性读）、#111/#92 退出码统一、#108 wait_api 拆分、#110/#112/#107/#113 收尾批、#118/#119 review 快修（PR #120）、#98 场景隔离 scene-reload/scene-change + fresh_scene（PR #121）、#102 时间控制 time-scale/pause/step-frames + 1009（PR #125）、#122/#123/#124 收尾批：bridge fixture 兜底还原 pause/time_scale + fresh_scene 1008 pitfall + GUT wait_frames 替换（PR #126，本批；顺手追平了仓内 .claude/skills 渲染版 SKILL.md——它不会自动刷新，改模板/CLI 后记得用 skills_install.render_skill 重渲染——必须 COLUMNS=80 + Python 3.12（argparse usage 折行随 Python 小版本变，CI skill-render-drift 以 3.12 为准））。#127/#128 CI 门禁批（PR #129）：ci-ok 聚合 job 上线，main required check 锚定它（gh pr merge --auto 现在真等绿）；skill-render-drift 防漂移检查上线。#101 视觉态断言（PR #130）：sprite-info（effective_region/frame_texture）+ screenshot --node 裁剪 + 1010/1011。#103 诊断三件套（PR #131）：errors 结构化查询（Logger 拦截 + seq 游标，需 Godot 4.5+，否则 1012）+ daemon logs --tail + pytest no_push_errors fixture / 失败自动截图。#109 wait_property 性能（PR #132）：免物化直比 + 属性存在性 memo + `_has_property` in-快拒；顺手修了 GDScript 跨类型 `==` 是运行期脚本错误（中止函数抛 Nil、非静默 false）的存量雷（`_safe_eq` 守卫）——改 wait 比较语义 / encode_value / _deep_equal 任意一处必须重跑 test_wait_api 的 38k parity 矩阵；引擎内部深比较类型严格（dict 内 int/float 不互通、不吃 tolerance），别按 GDScript 标量 `==` 直觉想当然。init 默认覆盖 addon（PR #133）：`run_init` 参数 force→clobber_addon（默认 True，与 clobber_skills 对称），重跑 init 即同步 addon+SKILL.md 到当前 CLI 版本；`--keep-addon` 是逃生口，`--force` 只是兼容 no-op（与 --keep-addon 互斥）——别再在文档/脚本里教人用 `--force`。PATH mono 别名（PR #134）：`find_godot_binary` PATH 候选名追加 `godot4-mono / godot-mono / godot_mono / Godot_mono`，排标准名后（双版并存优先轻量标准版）；macOS /Applications 与 Windows Program Files 两级 glob 天然覆盖 mono（`Godot_mono.app` 内部可执行文件仍叫 `Godot`，Homebrew cask 可证），零改动。#135 ~/Applications 兜底（PR #136）：macOS 扫描序抽成 `_macos_app_dirs()`（/Applications 优先、~/Applications 兜底），单测必须经它注入 tmp 目录——开发机真 /Applications/Godot.app 会抢先命中，不注入则用户级分支测不到。#137 screenshot --node hiDPI/stretch 裁剪错位（PR #138）：compute_node_screen_rect 漏乘 viewport final transform——rect 是画布（逻辑设计分辨率）坐标而取图侧 get_image() 是窗口物理像素，平窗两系重合所以长期未暴露；GUT 回归用 SubViewport size_2d_override(+stretch) 构造 scale≠1（headless 可测），下游 XGame Retina 实测验证。README 追平（PR #139）：python/README.md（即 PyPI description 源）与顶层 README 补齐 #96–#103 批命令面——PyPI 页面只在发版时刷新，改 README 不会立即生效。#140 CHANGELOG 欠账归档（PR #141）：[Unreleased] 按 `git tag --contains` 逐条归位（#96 批→0.2.15、#52/#54 批→0.2.5、AI-friendly 改造→0.2.0），补写 0.2.1–0.2.17 全部版本段；release.sh 新增门禁——[Unreleased] 非空拒绝打 tag，`--roll-changelog` 自动滚动为版本段（commit+push 后再 tag）。**以后变更一律记入 [Unreleased]，发版时自动归档**；改 release.sh 滚动逻辑记得同步 CHANGELOG 文件头的约定说明。仍 open：#88、#18（需 maintainer 操作）——code issue backlog 已清。下次 review 发现新坑请补到这里。）*
